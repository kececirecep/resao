        ...
        <div className={`${props.bgColor} py-10 sm:py-16 lg:py-20`}>
            ...
            <div className={`grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 container mx-auto ${props.animate ? 'wow animate__fadeIn' : ''}`} data-wow-duration="3.5s">
                {CounterData.map((item, index) => {
                    return (
                        <div key={index} className='  p-4 md:p-12 text-center m-2 group relative'>
                            <h2 className='pb-4 duration-500 text-5xl md:text-6xl text-red-500 flex items-start justify-center font-semibold'>
                            ...
                            <p className='duration-500 text-md md:text-lg text-gray-600'>{item.countTitle}</p>
                        </div>
                    );
                })}
            </div>
